...
boolean PS2X::NewButtonState() {
  return ((last_buttons ^ current_buttons) > 0);
}

/****************************************************************************************/
boolean PS2X::NewButtonState(unsigned int test_button) {
  return ( ( (last_buttons ^ current_buttons) & test_button) > 0);
}

/****************************************************************************************/
boolean PS2X::ButtonPressed(unsigned int test_button) {
  return(NewButtonState(test_button) & Button(test_button));
}

/****************************************************************************************/
boolean PS2X::ButtonReleased(unsigned int test_button) {
  return((NewButtonState(test_button)) & ((~last_buttons & test_button) > 0));
}

/****************************************************************************************/
boolean PS2X::Button(uint16_t test_button) {
  return ((~current_buttons & test_button) > 0);
}
...
/****************************************************************************************/
byte PS2X::Analog(byte test_button) {
   return PS2data[test_button];
}
...
boolean PS2X::read_gamepad(boolean motor1, byte motor2) {
   ...
   if (temp > 1500) reconfig_gamepad(); //waited to long
   if(temp < read_delay) delay(read_delay - temp);//waited too short

   if(motor2 != 0x00) motor2 = map(motor2,0,255,0x40,0xFF); //noting below 40 will make it spin
   ...
   for (byte RetryCnt = 0; RetryCnt < 5; RetryCnt++) {
      ...
      delayMicroseconds(CTRL_BYTE_DELAY);
      //Send the command for button and joystick data;
      ...
      if(PS2data[1] == 0x79) {  //if controller is in full data return mode, get the rest of data
         for (int i = 0; i<12; i++) PS2data[i+9] = _gamepad_shiftinout(dword2[i]);
      }
   ...
   Serial.print("    ");
   Serial.println(read_delay, DEC);
   Serial.println(" mS");
#endif

   last_buttons = current_buttons; //store the previous buttons states
   current_buttons =  (uint16_t)(PS2data[4] << 8) + PS2data[3];   //store as one value for multiple functions
...
byte PS2X::config_gamepad(uint8_t clk, uint8_t cmd, uint8_t att, uint8_t dat, bool pressures, bool rumble) {
  ...
  CLK_SET(); // SET(*_cclk_oreg,_clk_mask);
...
byte PS2X::readType() {
  ...
  if (temp[3] == 0x03) return 1;  // DualShock
  else if (temp[3] == 0x01) return 2; // GuitarHero
  else if (temp[3] == 0x0C) return 3; // Wireless Sony DualShock
